...
> Ошибки (например, дубликат `ID`) показываются на экране.<br>
> При удалении категории, банк аккаунта не удаляется операция связанная с этими объектами. При этом операции продолжает существовать с этой операцией и аккаунтом
---
